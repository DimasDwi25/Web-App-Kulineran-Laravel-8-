            ...
            <div class="card">
                <div class="card-body">
                    <div class="row  mb-2 text-center">
                        ...
                        <div class="card-header">{{ $key->nama_bank }}</div>
                        <div class="card-body">
                        <h5 class="card-title">{{ $key->no_rekening }}</h5>
                        <p class="card-text">Atas Nama {{ $key->name }}</p>
                        </div>
                        </div>
                        </div>
                        @endforeach
                    ...
                    <div class="row  mb-4">
                        <div class="col-md-12 text-center">
                            Transfer Sebesar Rp {{ number_format($order->sub_total,2,',','.') }} Ke No Rekening Di Atas
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                    ...
                    <div class="col-md-6">
                        <form action="{{ route('user.order.kirimbukti',$order->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
